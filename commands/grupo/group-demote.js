let handler = async (m, { conn, text }) => {
  ...
  if (!text && !m.quoted && !m.mentionedJid?.length) {
    return conn.reply(m.chat, `🪐 *Debes responder, mencionar o escribir un número para quitar administrador.*`, m, { quoted: fgrupo })
  }
  ...
  if (conn.user.jid.includes(user)) return m.reply(`✳️ No puedo auto-quitarme administrador`, fgrupo)
  ...
  try {
    ...
    if (user === ownerGroup) return m.reply(`✳️ No puedo quitar al dueño del grupo`, fgrupo)
    if (user === ownerBot) return m.reply(`✳️ No puedo quitar al dueño del bot`, fgrupo)
...
await conn.sendMessage(m.chat, {
  ...
  mentions: [user]
    ...
    await m.react('❌')
    m.reply(`❌ Ocurrió un error al intentar quitar administrador`, fgrupo)
  }
}
