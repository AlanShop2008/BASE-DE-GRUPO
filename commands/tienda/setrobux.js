...
const handler = async (msg, { conn, args, text }) => {
  ...
  if (!textoCrudo && !quotedText && !quotedImage) {
    return conn.sendMessage(
      chatId,
      { text: "✏️ Usa el comando así:\n\n• *setrobux <texto>* (multilínea permitido)\n• O responde a una *imagen* con: *setrobux <texto>*" },
      { quoted: msg }
    );
  }
  ...
  if (quotedImage) {
    try {
      ...
      console.error("[setrobux] error leyendo imagen citada:", e);
    }
  }
  ...
  await conn.sendMessage(chatId, { text: "✅ *Robux actualizado con éxito.*" }, { quoted: msg });
};


handler.help = ['setrobux']
handler.tags = ['ventas']
handler.command = ["setrobux"];
