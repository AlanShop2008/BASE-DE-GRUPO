...
const handler = async (msg, { conn }) => {
  ...
  const data = ventas[chatId]?.setrobux;

  if (!data || (!data.texto && !data.imagen)) {
    return conn.sendMessage(chatId, { text: "❌ No hay contenido guardado con setrobux en este grupo." }, { quoted: msg });
  }
  ...
  if (data.imagen) {
    ...
    await conn.sendMessage(chatId, {
      image: buffer,
      caption: data.texto || "Contenido Robux"
    }, { quoted: msg });
  } else {
