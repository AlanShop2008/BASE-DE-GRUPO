...
const handler = async (msg, { conn, args, text }) => {
  ...
  if (!textoCrudo && !quotedText && !quotedImage) {
    return conn.sendMessage(
      chatId,
      { text: "✏️ Usa el comando así:\n\n• *setdiamantes <texto>* (multilínea permitido)\n• O responde a una *imagen* con: *setdiamantes <texto>*" },
      { quoted: msg }
    );
  }
  ...
  if (quotedImage) {
    try {
      ...
      console.error("[setdiamantes] error leyendo imagen citada:", e);
    }
  }
...
handler.help = ['setdiamantes + texto']
handler.tags = ['ventas']
