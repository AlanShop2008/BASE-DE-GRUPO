...
let totalUsers = Object.values(global.db.data.users)
  .filter(u => u.exp > 0).length

let totalPremium = Object.values(global.db.data.users)
...
> 👊🏻 Bienvenido al sistema de ventas de *${botname}*
> 🛍️ Aquí encontrarás todos los comandos premium de ventas
...
const fgrupo = {
  ...
  message: {
    locationMessage: {
      name: groupName,
      jpegThumbnail: ppUrl
        ? await (await fetch(ppUrl)).buffer()
        : null
    }
  }
}
